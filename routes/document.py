...
def admin_list():
    ...
    # [대시보드 통계용 원본 수치 저장]
    global_total = len(df)
    global_pending = len(df[df['상태'] == '대기'])
    
    # ⭐ [추가된 부분: 증명서 종류별 통계]
    count_career = len(df[df['증명서종류'] == '경력증명서'])
    count_employment = len(df[df['증명서종류'] == '재직증명서'])
    count_dismissal = len(df[df['증명서종류'] == '해촉증명서'])
    count_activity = len(df[df['증명서종류'] == '강사활동증명서'])
    count_inst_dismissal = len(df[df['증명서종류'] == '강사해촉증명서'])
    count_excellent = len(df[df['증명서종류'] == '우수강사인증서'])
    ...
    if search_keyword:
        mask = (
            df_with_idx['성명'].str.contains(search_keyword, case=False) |
            df_with_idx['근무장소'].str.contains(search_keyword, case=False) |
            df_with_idx['강의과목'].str.contains(search_keyword, case=False) |
            df_with_idx['용도'].str.contains(search_keyword, case=False) |
            df_with_idx['증명서종류'].str.contains(search_keyword, case=False) |
            df_with_idx['직책'].str.contains(search_keyword, case=False)
        )
        df_with_idx = df_with_idx[mask]
    ...
    per_page = 10
    filtered_count = len(submissions_all)
    ...
    return render_template('certificate/admin.html', 
                           ...
                           count_dismissal=count_dismissal,
                           count_activity=count_activity,               # 증명서 별 숫자통계들 보여주는 카운트 함수
                           count_inst_dismissal=count_inst_dismissal,  
...
def send_email_to_instructor(to_email, name, pdf_path, cert_type):
    """생성된 PDF를 첨부하여 강사/임직원에게 메일 발송 (yagmail -> smtplib 587 이중 시도)"""
    ...
    subject = f"[(사)새담] 요청하신 {cert_type} 발송 안내 ({name} 님)"
...
def send_admin_alert(name, cert_type, role="강사님"):
    """신청 발생 시 관리자에게 간단 알림"""
    email_addr, email_pw = get_email_credentials()
