...
- 계약 등록·발송은 운영 서비스 `https://works.saedam.org`에서 진행합니다.
  계약 링크는 내부 프록시 주소와 관계없이 이 HTTPS 도메인으로 생성되며,
  해당 계약이 운영 DB에 있어야 합니다.
- 통합관리의 솔라피설정에 승인된 채널 PF ID와 템플릿 ID를 저장합니다.
  본문은 `#{이름}`, PC·모바일 웹링크는 `https://#{url}`을 사용하는 템플릿입니다.
  `#{url}`에는 시스템이 `https://`를 제외한 주소를 전달합니다.
- 본문·강조문구·버튼은 승인 템플릿을 사용합니다. API의 접수 실패 목록과
  상태 코드를 확인해 오류를 표시하며, 발송 접수는 최종 수신 완료와 다릅니다.
- 읽기 전용 템플릿 점검: `python scripts/diagnose_contract_alimtalk.py --template-check`
  (메시지 실발송 없이 승인 템플릿과 치환 결과를 비교합니다.)
